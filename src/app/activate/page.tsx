...
const ActivatePage: React.FC = () => {
  ...
  const onSubmit = async (data: ActivateFormData) => {
    try {
      ...
      await apiService.activateSeller(data);
      ...
      setActivateError(
        error.response?.data?.message || 'Erro ao ativar conta. Verifique o código e tente novamente.'
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit(onSubmit)}>
          <div className="space-y-4">
            ...
            <div>
              <label htmlFor="codigoAtivacao" className="block text-sm font-medium text-gray-700">
                Código de Ativação
              </label>
              <input
              ...
              {errors.codigoAtivacao && (
                <p className="mt-1 text-sm text-red-600">{errors.codigoAtivacao.message}</p>
              )}
            </div>
          </div>
